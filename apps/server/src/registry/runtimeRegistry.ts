...
import { getRuntimeCtorRegistryManifest } from "@shared/content/catalog.ts";
import type {
...
type RuntimeCtorBySymbol<TCtor> = Readonly<Record<string, TCtor>>;

function resolveRuntimeCtors<TCtor>(
  entries: readonly { readonly symbol: string }[],
  ctorsBySymbol: RuntimeCtorBySymbol<TCtor>,
  registryName: string,
): readonly TCtor[] {
  return entries.map((entry) => {
    const ctor = ctorsBySymbol[entry.symbol];
    if (!ctor) {
      throw new Error(
        `Unknown ${registryName} runtime constructor symbol ${entry.symbol}.`,
      );
    }
    return ctor;
  });
...
const entityRuntimeCtorBySymbol = {
  Player,
  ItemEntity,
  Bomber,
  Commander,
  ...
  ThanosBullet,
  ThanosRocket,
} satisfies RuntimeCtorBySymbol<RegistrableEntityCtor>;

const itemRuntimeCtorBySymbol = {
  BaseballBat,
  BasicDagger,
  BasicGun,
  BasicRifle,
  ...
  StructureFenceVItem,
  StructureTreeItem,
} satisfies RuntimeCtorBySymbol<RegistrableItemCtor>;

const effectRuntimeCtorBySymbol = {
  BleedingEffect,
  ConfusionEffect,
  DamageEffect,
  FracturedEffect,
  KnockbackEffect,
  StunnedEffect,
} satisfies RuntimeCtorBySymbol<RegistrableEffectCtor>;

const runtimeCtorRegistryManifest = getRuntimeCtorRegistryManifest();

export const entityRuntimeCtors = resolveRuntimeCtors(
  runtimeCtorRegistryManifest.entityRuntimeCtors,
  entityRuntimeCtorBySymbol,
  "entity",
);

export const itemRuntimeCtors = resolveRuntimeCtors(
  runtimeCtorRegistryManifest.itemRuntimeCtors,
  itemRuntimeCtorBySymbol,
  "item",
);

export const effectRuntimeCtors = resolveRuntimeCtors(
  runtimeCtorRegistryManifest.effectRuntimeCtors,
  effectRuntimeCtorBySymbol,
  "effect",
);
